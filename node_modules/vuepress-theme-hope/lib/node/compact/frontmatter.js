 ...
 */
export const convertFrontmatter = (frontmatter, filePathRelative = "") => {
    DEPRECATED_FRONTMATTER_OPTIONS.forEach(([deprecatedOption, newOption]) => deprecatedLogger({
        options: frontmatter,
        deprecatedOption,
        newOption,
        scope: `${filePathRelative || ""} frontmatter`,
    }));
    DROPPED_FRONTMATTER_OPTIONS.forEach((item) => droppedLogger(frontmatter, ...item, `${filePathRelative ? `Found in ${filePathRelative}` : ""}`));
    if ("meta" in frontmatter) {
        logger.warn(`"meta" in frontmatter is deprecated in V2, please use "head" instead.${filePathRelative ? `Found in ${filePathRelative}` : ""}`);
        frontmatter["head"] = [
            ...(frontmatter["head"] || []),
    ...
    if ("canonicalUrl" in frontmatter) {
        logger.warn(`"canonicalUrl" in frontmatter is deprecated, please use "head" instead.${filePathRelative ? `Found in ${filePathRelative}` : ""}`);
        frontmatter["head"] = [
            ...(frontmatter["head"] || []),
    ...
    if (frontmatter["home"] === true) {
        if (frontmatter["layout"] === "Blog") {
            logger.warn(`"layout: Blog" in frontmatter is deprecated, please use "layout: BlogHome" instead.${filePathRelative ? `Found in ${filePathRelative}` : ""}`);
            frontmatter["layout"] = "BlogHome";
        }
        // check project homepage
        if (!("layout" in frontmatter))
            DEPRECATED_HOME_FRONTMATTER_OPTIONS.forEach(([deprecatedOption, newOption]) => deprecatedLogger({
                options: frontmatter,
                deprecatedOption,
                newOption,
                scope: `${filePathRelative || ""} frontmatter`,
