...
const handleBlogOptions = (blogOptions) => {
    ...
    if ("autoExcerpt" in blogOptions) {
        logger.error('"blog.autoExcerpt" options is no longer supported, please use "plugins.blog.excerptLength" instead');
 ...
 * @deprecated You should use V2 standard options and avoid using it
 */
export const convertThemeOptions = (themeOptions) => {
    // ensure plugins
    ...
    if (isArray(plugins["components"])) {
        logger.warn('"plugins.components" no longer accepts array, please set it to "plugin.components.components" instead.');
        plugins["components"] = {
        ...
        Object.values(themeOptions["locales"]).forEach((localeConfig) => {
            ...
            if (isPlainObject(localeConfig["navbarLayout"])) {
                if ("left" in localeConfig["navbarLayout"]) {
                    logger.warn('To have better meaning under RTL layout, "left" option is deprecated, please usee "start" instead');
                    localeConfig["navbarLayout"]["start"] = localeConfig["navbarLayout"]["left"];
                }
                if ("right" in localeConfig["navbarLayout"]) {
                    logger.warn('To have better meaning under RTL layout, "right" option is deprecated, please usee "end" instead');
            ...
            if (isPlainObject(localeConfig["blog"]) && localeConfig["blog"]) {
                ...
                if (!plugins["blog"])
                    logger.warn('Blog feature is tree-shakable in v2, you should set "plugins.blog: true" in theme options to enable it.');
            }
        });
    }
    return themeOptions;
